init_PUMMM <-
  function(X,
    ...
    } else if (initG == "ManlyMix"){
      w <- tryCatch(
        {
          suppressWarnings(mclust::unmap(ManlyMix::Manly.model(X, G)$Model$id))
        },
        error = function(e) {
          mclust::unmap(stats::kmeans(X, centers = G, iter.max = 100)$cluster)
        }
      )
    }
    ...
    if (is.null(lambda)){
      lambda <- tryCatch(
        {
          suppressWarnings(t(ManlyMix::Manly.model(X, G)$Model$la))
        },
        error = function(e) {
          matrix(0.1, nrow = p, ncol = G)
        }
      )
      model$lambda = lambda
    } else {
    ...
    Y = manly_transform_mat(X, model$lambda)
    model$post <- post_puMmm(X, Y, model$pp, model$lambda, model$mu, model$Sigma)
    return(list(
      label = label,
      model = model,
      Y = Y,
