PathDiagram <- function (p, m, V, outV, A, B, levfus, Sb, Sw, Sv, g) {
  nnode <- (m * 2) - 1 + p
  fnode <- m + p
  ...
  G <- igraph::add.vertices(G, fnode)
  for (i in 1:length(t)) {
    G <- igraph::add.edges(G, c(t[i], tt[i]))
  }
  ...
  G <- igraph::add.vertices(G, m - 1)
  for (q in (m - 1):1) {
    G <- igraph::add.edges(G, c(m + p + d, tabnode1[q, 1], m + p + d, tabnode1[q, 2]))
      ...
      if (v  %in% igraph::V(G)) {
        ...
        if (length(z) > 0) {
          ...
          if (length(vertices_to_delete) > 0) {
            ...
            if (any(!is.na(vlabels[x[z]]))) {
              notna <- which(!is.na(vlabels[x[z]]))
              newlabel <- vlabels[x[z][notna]]
              vlabels[v] <- newlabel
            }
  ...
  while (swap > 0) {
    ...
    for (i in 1:(length(igraph::E(G)) - 1)) {
      for (j in i:length((igraph::E(G)))) {
        ...
        if (length(x_values) == length(unique(x_values)) &&
            length(y_values) == length(unique(y_values))) {
...
IntersectEdges <- function(p1, p2, q1, q2) {
  b <- q1 - p1
  A <-
    cbind(p2 - p1, q1 - q2)  # nonsingular when segments have different slopes
